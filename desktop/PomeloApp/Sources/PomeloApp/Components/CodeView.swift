...
struct CodeModel {
    ...
    let starts: [Int]
    let lineBg: [NSColor?]
    /// Per-line margin text. Empty for views that keep numbers inside the string;
    /// when set, it is painted in the inset so a selection cannot pick it up.
...
final class CodeTextView: NSTextView {
    static let diffGutterWidth: CGFloat = 88
    ...
    // wraps: soft-wrap long lines instead of scrolling sideways. Opt-in — the
    // side-by-side and peek views need a fixed-width container to stay aligned.
    func configureReadOnly(inset: NSSize, wraps: Bool = false) {
        isEditable = false
        isSelectable = true
        isRichText = false
    ...
    override func drawBackground(in rect: NSRect) {
        ...
        drawGutters(lm: lm, tc: tc, storage: storage, inset: inset)
    }

    // Painted in the left inset rather than appended to the string: a selection can
    // only cover text, so the copied result is the code alone.
    private func drawGutters(lm: NSLayoutManager, tc: NSTextContainer, storage: NSTextStorage, inset: CGFloat) {
        ...
        for (i, cell) in gutters.enumerated() where i < lineStarts.count {
            let firstChar = lineStarts[i]
            let glyphRange = lm.glyphRange(forCharacterRange: NSRange(location: firstChar, length: 0),
                                           actualCharacterRange: nil)
            // Only the first fragment of a wrapped line carries its numbers.
            var r = lm.lineFragmentRect(forGlyphAt: glyphRange.location, effectiveRange: nil)
            r.origin.y += inset
            func draw(_ text: String, _ x: CGFloat, _ w: CGFloat, _ color: NSColor) {
                guard !text.isEmpty else { return }
                let str = NSAttributedString(string: text, attributes: [.font: font, .foregroundColor: color])
                str.draw(at: NSPoint(x: x + w - str.size().width, y: r.origin.y + 3))
            }
            draw(cell.old, 4, 32, dim)
            draw(cell.new, 40, 32, dim)
            draw(cell.sign, 76, 8, cell.signColor)
        }
    }
}
