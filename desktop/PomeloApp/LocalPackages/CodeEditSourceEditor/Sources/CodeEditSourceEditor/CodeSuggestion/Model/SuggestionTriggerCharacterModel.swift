...
final class SuggestionTriggerCharacterModel {
    ...
    func textView(_ textView: TextView, didReplaceContentsIn range: NSRange, with string: String) {
        ...
        let caret = mutation.postApplyRange.max
        let contents = textView.textStorage.string as NSString
        var presentChar: Character?
        if caret > 0, caret <= contents.length {
            presentChar = contents.substring(with: NSRange(location: caret - 1, length: 1)).last
        }
        guard let lastChar = presentChar,
              triggerCharacters.contains(String(lastChar)) || lastChar.isNumber || lastChar.isLetter || lastChar == "_" else {
            lastPosition = nil
            return
        }
